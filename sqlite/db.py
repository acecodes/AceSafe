...
class DB_drone:
    ...
    def routines_insert(self, database, table, *routines):
        ...
        cursor.execute(
            '''CREATE TABLE {0} (Routine TEXT PRIMARY KEY);'''.format(table))
        ...
        for items in routines:
            ...
            try:
                cursor.execute(
                    '''INSERT INTO {0} (Routine) VALUES (?)'''.format(table), (items,))
    ...
    # Generates a menu for use in the command prompt/bash shell and allows a
    # user to select a syncing routine
    def create_menu(self, database, just_view=False):
        # Connect to the database and establish a cursor
        conn = sqlite3.connect(database, isolation_level=None)
        cursor = conn.cursor()

        # sqlite_master calls all tables from a single database file, as
        # opposed to a normal "SELECT * FROM (table)" call
        ...
        try:
            ...
            pass
        # Execute the routine, then return to the main menu (even if there are
        # problems)
